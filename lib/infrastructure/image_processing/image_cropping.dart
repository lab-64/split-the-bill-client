...
typedef PerspectiveCroppingFunctionNative = Uint32 Function(Pointer<ffi.Utf8>,
    Pointer<NativeDetectedRectangle>, Pointer<Pointer<Uint8>>);
typedef PerspectiveCroppingFunction = int Function(Pointer<ffi.Utf8>,
    Pointer<NativeDetectedRectangle>, Pointer<Pointer<Uint8>>);

class ImageCropping {
  static Future<Uint8List> perspectiveImageCropping(
      String imgPath, DetectedRectangle cropPath) async {
    final perspectiveCropping = nativeImageCropping.lookupFunction<
        PerspectiveCroppingFunctionNative,
        PerspectiveCroppingFunction>("perspectiveTransform");

    Pointer<Pointer<Uint8>> outputBuffer =
        ffi.malloc.allocate(sizeOf<Pointer<Uint8>>());
    final imgPathNative = imgPath.toNativeUtf8();

    final Pointer<NativeDetectedRectangle> cropPathPtr =
        ffi.malloc.allocate(sizeOf<NativeDetectedRectangle>());
    ...
    int outputSize =
        perspectiveCropping(imgPathNative, cropPathPtr, outputBuffer);
    if (outputSize == 0) {
      log("decoded image was null");
      return Uint8List(0);
    }
    Uint8List projectedBytes =
        Uint8List.fromList(outputBuffer.value.asTypedList(outputSize));
