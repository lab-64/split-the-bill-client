...
class GroupSelectionScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final isTutorialSeen =
        ref.watch(tutorialControllerProvider(TutorialScreen.newBillSelection));

    if (!isTutorialSeen) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        showTutorialDialog(
          context: context,
          screen: TutorialScreen.newBillSelection,
          onDismiss: () => ref
              .read(tutorialControllerProvider(TutorialScreen.newBillSelection).notifier)
              .markAsSeen(),
        );
      });
    }
